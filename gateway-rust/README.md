...
> **状态：默认网关实现。** 本 crate 是 Rust 原生网关，当前实现 **HTTP 骨架 + SQLite 持久化 + 渠道/分组 CRUD + 请求日志 list/clear + 非流式/SSE 流式 Chat 转发**。本地桌面场景 **无鉴权**（管理 API 与 `/v1/*` 均开放）。  
...
- **本地无鉴权**：`/api/v1/*` 与 `/v1/*` 均不强制 Token（默认绑定 `127.0.0.1`）
...
| `/api/v1/*` | 按业务（不要求 JWT） |
| `/v1/models` | 200 |
| `/v1/chat/completions` | 按业务（不要求 API Key） |
...
- 无凭证访问管理 API 与 `/v1/models`
- 渠道 CRUD → 分组 CRUD → models（分组名列表）
- wiremock 上游 200 转发、SSE 多帧 + `[DONE]`、未知分组、轮询
- mock chat 后 log list / clear / page_size 上限
