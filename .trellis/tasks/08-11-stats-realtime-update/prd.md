...
- [ ] AC1：通过代理发起一次成功请求后，首页统计卡片在**事件驱动下立即**（无需等 5 秒轮询）反映新数据；请求次数 +1 可见。
- [ ] AC2：主窗口从托盘恢复可见时，统计卡片立即拉取最新数据（无需等下一个轮询 tick）。
- [ ] AC3：5 秒兜底轮询保留，事件监听失败/丢失时统计仍会更新。
- [ ] AC4：`pnpm typecheck`、`cargo build` 通过；Rust 侧新增逻辑不破坏现有测试（`cargo test`），并为变更订阅新增单元测试。
