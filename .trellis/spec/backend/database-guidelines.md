...
- migrate 幂等；重启后 `find_by_raw_key` / list 一致。
- 渠道 `type`、分组 `mode` 以数字进出 JSON。
- `cargo test --manifest-path gateway-rust/Cargo.toml` 含临时库 CRUD 与跨 store 实例持久化。
