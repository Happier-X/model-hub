...
| `model_pricing` | OpenRouter 同步的模型单价（`model_name` 主键、`prompt_price_per_mtok`/`completion_price_per_mtok` 每百万 token 美元、`updated_at`）；未覆盖模型无行（视为 0 价）；同步为全量 replace（upsert + 清理过期） |
