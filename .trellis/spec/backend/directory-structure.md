...
- 鉴权（实验已实现）：
  - 管理：`POST /api/v1/user/login`（默认 admin/admin）签发 HS256 JWT；`GET /api/v1/user/status` 与 `/api/v1/apikey/*` 需管理 JWT。
  ...
  - API Key 前缀固定 `sk-octopus-`；完整明文仅 create 返回一次；`ApiKeyStore` 只存哈希 + 脱敏元数据（当前 `MemoryApiKeyStore`）。
  - 日志禁止打印完整 Token/Key。
- 生命周期：通过预绑定 `TcpListener` + graceful shutdown；测试只绑定 `127.0.0.1:0`，不结束任何 octopus 进程。
- 模块边界：`config` / `http` / `server` / `error` / `auth` / `apikey` / `routes` / `response` 保持可测试；后续 SQLite、渠道和 Chat 通过 router/state 扩展，不把业务塞进 Tauri 壳。
