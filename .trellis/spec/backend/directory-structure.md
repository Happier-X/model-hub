...
- 配置：只消费 `server.host` / `server.port`，代码默认值为 `127.0.0.1:8080`；host 当前仅接受 IP，端口不得为 0。
...
- 模块边界：`config` / `http` / `server` / `error` 保持可测试；后续鉴权、SQLite、渠道和 Chat 通过 router/state 扩展，不把业务塞进 Tauri 壳。
