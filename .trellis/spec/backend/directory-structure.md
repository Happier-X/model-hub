...
- 鉴权与持久化（实验已实现）：
  ...
  - 客户端：`POST /v1/chat/completions` 非流式转发；请求 `model`=分组名，上游 `model`=item.`model_name`；`stream=true` → 400 `STREAM_NOT_SUPPORTED`；上游状态码/body 透传；网络失败 502。
...
- 模块边界：`config` / `http` / `server` / `error` / `db` / `auth` / `apikey` / `channel` / `group` / `router` / `upstream` / `routes` / `response` 保持可测试；SSE 流式留给后续子任务，不把业务塞进 Tauri 壳。
