...
- 模块边界：`config` / `http` / `server` / `error` / `db` / `auth` / `apikey` / `channel` / `group` / `router` / `upstream` / `routes` / `response` 保持可测试；SSE 由 `upstream::forward_chat_stream` 透明代理，不把业务塞进 Tauri 壳。
