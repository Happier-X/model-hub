...
5. **客户端网关 Key 必填**：钉扎侧车 v0.9.28 与 `gateway-rust` 实验实现对 `/v1/*` 强制 `sk-octopus-...`；管理 JWT 与客户端 Key 分离（管理 JWT 不得当作 `/v1/*` 凭证）。不 patch 侧车做「本机免鉴权」；文档/UI 禁止写「任意占位 api_key」。完整 Key 不得明文持久化（内存/文件均只存哈希）。
