...
5. **队列耗尽**：最后一次失败若有上游 HTTP 响应，首选透传该候选项原始状态、响应头与错误体；但若最后响应是 `2xx` 结构化错误信封（HTTP 200 + 裸 JSON 错误体，或 HTTP 200 + SSE 帧内 `data:` payload 错误信封），则不应当原样透传误导客户端，应升级为 `502` + 汇总错误摘要。若最后一次无上游响应（网络/超时/读失败），返回明确网关错误（如 `502`/`504` + 摘要）。
...
- `2xx` 错误信封仅认结构化字段：字符串 `error`、对象 `error.message`、`type: "error"`、顶层 `message` 等。形态覆盖**裸 JSON**与**SSE 帧内 `data:` payload**（`is_structured_error_body` 先剥 `data:` 再复用同一 JSON 判定）。正常 chat completion（含 `choices`）、`data: [DONE]`、纯注释 / 仅 `event:`/`id:` 心跳、SSE 内非 JSON 文本**不换源**。
...
| 明确 `2xx` 错误信封（裸 JSON） | 换源 |
| 明确 `2xx` SSE 帧内错误信封（`data: {"error":...}` 等） | 换源；首包不提交客户端 |
| 正常 `2xx` completion / 正常 SSE（含 `choices`/`delta`/`[DONE]`/注释） | 提交响应，不换源 |
...
- 单元：`is_structured_error_body` 正反例（裸 JSON 错误信封 vs completion；SSE 帧内错误信封 vs 正常 delta/`[DONE]`/注释）。
- 集成：流式 SSE 帧错误信封首包前换源；正常 SSE 不换源；SSE 帧错误耗尽升级 502。
- 集成：流式首包后静默超时单日志、不换源。
