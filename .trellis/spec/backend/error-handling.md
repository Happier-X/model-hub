...
5. **队列耗尽**：最后一次失败若有上游 HTTP 响应，首选透传该候选项原始状态、响应头与错误体；但若最后响应是 `2xx` 结构化错误信封（HTTP 200 + JSON 错误体），则不应当原样透传误导客户端，应升级为 `502` + 汇总错误摘要。若最后一次无上游响应（网络/超时/读失败），返回明确网关错误（如 `502`/`504` + 摘要）。
