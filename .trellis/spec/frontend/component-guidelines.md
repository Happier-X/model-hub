...
11. **上游访问**：禁止供应商页「测试连接」及任何自动/后台对用户上游的测活；分组页「拉取模型」**仅**用户点击触发，不得在 `onMounted`/保存时自动拉取。健康展示只读熔断内存（`listHealth`），不打上游。合同见 backend [upstream-access.md](../backend/upstream-access.md)。
