...
4. （可选）在「API 密钥」页创建客户端 Key；本机默认可不创建。对接 Pi 时在「分组」页对目标分组点「配置到 Pi」（无需 Key）。
...
期望返回 HTTP 200，`data[].id` 是已配置的分组名。携带错误或已停用的 Key 应返回 HTTP 401；占位 `model-hub` 与无 Key 同等放行。
...
| HTTP 401 | 携带了错误、禁用或已删除的 Key（非占位） | 去掉错误 Key，或创建/启用正确客户端 Key；确认已重启到新版本代理 |
| Pi 仍 401 | 仍在跑旧二进制，或 models.json 里写了假 Key | 托盘完全退出后重开；在分组页重新「配置到 Pi」（写入占位 `model-hub`）或去掉错误 Key |
