...
| 网关客户端 Key | 应用 **API 密钥** 页创建的 `sk-modelhub-...` |
...
4. **API 密钥** → 创建并**完整复制一次** `sk-modelhub-...`（与设置页管理 JWT **不是**同一套）。 
...
| **401** Authentication failed | 占位 Key、管理 JWT、Key 禁用/过期 | API 密钥页创建并复制完整 `sk-modelhub-...` |
